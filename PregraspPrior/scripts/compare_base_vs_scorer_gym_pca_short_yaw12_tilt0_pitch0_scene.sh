...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" python3 pregrasp_eval/eval_yaw_scorer_SE_residual.py \
  ...
  --tilt_angles=0 \
  --pitch_angles=0 \
  +pregrasp_yaw_scorer_checkpoint=/mnt/AAAI2027_grasp/PregraspPrior/data/models/scorer_mlp_pca_short_yaw12_tilt0_pitch0_scene/best.pt \
  ...
  +pregrasp_yaw_compare_base_rounds="${BASE_ROUNDS:-1}" \
  +pregrasp_yaw_compare_scorer_rounds="${SCORER_ROUNDS:-3}" \
  +pregrasp_yaw_compare_base_mode=yaw0 \
