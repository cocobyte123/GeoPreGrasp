...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" python3 -m build_Pregrasp_data.yaw_pregrasp.collect.collect_yaw_pregrasp_rollouts \
  ...
  headless=true force_render=false num_envs="${NUM_ENVS:-3200}" \
  ...
  +yaw_pregrasp_collect_max_batches="${BATCHES:-50}" \
  +yaw_pregrasp_collect_overwrite_root=True \
  +yaw_pregrasp_collect_rollout_root=/mnt/AAAI2027_grasp/PregraspPrior/data/rollouts/pca_short_yaw12_tilt0_pitch0_scene \
